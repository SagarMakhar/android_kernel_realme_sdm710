...
void ohm_action_trig(int type)
{
        ...
        if (OHM_MEM_MON == type || OHM_SCHED_FSYNC == type || OHM_SCHED_SCM == type
                || OHM_MEM_LMK == type || OHM_MEM_VMA_ALLOC_ERR == type) {
                if (!ohm_kobj) {
                        ohm_err("kobj NULL\n");
                        return;
                }
...
#define OHM_CTRL_SCM       		BIT(OHM_SCHED_SCM)
#define OHM_CTRL_DSTATE         BIT(OHM_SCHED_DSTATE)
#define OHM_CTRL_SCHEDTOTAL     OHM_CTRL_EMMCIO | OHM_CTRL_FSYNC | OHM_CTRL_SCHEDLATENCY | OHM_CTRL_IOWAIT | OHM_CTRL_SCM | OHM_CTRL_DSTATE
...
#define OHM_CTRL_MEM_LMK        BIT(OHM_MEM_LMK)
#define OHM_MEM_VMA_ALLOC_ERR        BIT(OHM_MEM_VMA_ALLOC_ERR)
...
bool ohm_mem_lmk_trig = false;
bool ohm_mem_vma_alloc_err = false;
...
void ohm_trig_init(void)
{
        ...
        ohm_mem_lmk_trig = (ohm_trig_list & OHM_CTRL_MEM_LMK) ? true : false;
        ohm_mem_vma_alloc_err = (ohm_trig_list & OHM_MEM_VMA_ALLOC_ERR) ? true : false;
	for (i = 0 ;i < OHM_SCHED_TOTAL ;i++) {
...
/*scm call*/
static ssize_t scmcall_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
                char page[1024] = {0};
                int len = 0;
                int type = OHM_SCHED_SCM;

                len = sprintf(page, "scmcall_low_thresh_ms: %d\n""scmcall_high_thresh_ms: %d\n"
                                          "scmcall_low_cnt: %lld\n""scmcall_high_cnt: %lld\n"
                                          "scmcall_total_ms: %lld\n""scmcall_total_cnt: %lld\n"
                                          "scmcall_fg_low_cnt: %lld\n""scmcall_fg_high_cnt: %lld\n"
                                          "scmcall_fg_total_ms: %lld\n""scmcall_fg_total_cnt: %lld\n"
                                          "scmcall_fg_max_ms: %lld\n""scmcall_delta_ms: %lld\n"
                                          "scmcall_ctrl: %s\n""scmcall_logon: %s\n""scmcall_trig: %s\n",
                                          oppo_sched_para[type].low_thresh_ms,
                                          oppo_sched_para[type].high_thresh_ms,
                                          oppo_sched_para[type].low_cnt,
...
static const struct file_operations proc_scmcall_fops = {
       .read = scmcall_read,
};
...
/* warning: different project with different setting of LOW/HIGH/NUM */
#define LTT_LOW (748800)
#define LTT_HIG (1324800)
#define BIG_LOW (979200)
#define BIG_HIG (1843200)
#define LTT_NUM (6)
...
void update_stuck_trace_info(struct task_struct *tsk, int trace_type, unsigned int cpu, u64 delta) {
    ...
    if (trace_type == UIFIRST_TRACE_RUNNABLE) { // runnable
        ...
        if (cpu < LTT_NUM) {
            if (cur_freq <= LTT_LOW) {
                tsk->oppo_stuck_info.ltt_running_state[0] += delta;
            } else if (cur_freq <= LTT_HIG) {
                tsk->oppo_stuck_info.ltt_running_state[1] += delta;
            } else {
                tsk->oppo_stuck_info.ltt_running_state[2] += delta;
            }
        } else {
            if (cur_freq <= BIG_LOW) {
                tsk->oppo_stuck_info.big_running_state[0] += delta;
            } else if (cur_freq <= BIG_HIG) {
                tsk->oppo_stuck_info.big_running_state[1] += delta;
            } else {
                tsk->oppo_stuck_info.big_running_state[2] += delta;
            }
        }
    }
}
...
/******  OLMK & VMA_ALLOC_ERR  ******/
int vma_alloc_order = 9;
EXPORT_SYMBOL_GPL(vma_alloc_order);
unsigned long vma_alloc_err_counts;
EXPORT_SYMBOL_GPL(vma_alloc_err_counts);
unsigned long vma_alloc_counts;
EXPORT_SYMBOL_GPL(vma_alloc_counts);
unsigned long vma_alloc_err[10];
EXPORT_SYMBOL_GPL(vma_alloc_err);

static ssize_t vma_alloc_err_proc_read(struct file *file, char __user *buf,
		size_t count,loff_t *off)
{
	char page[256] = {0};
	int len = 0, len_tmp = 0, i = 0;


	len = sprintf(&page[len_tmp],"vma_alloc_counts:%lu\nvma_alloc_err_counts:%lu\n",
				vma_alloc_counts,
				vma_alloc_err_counts);
    len_tmp += len;
    len = sprintf(&page[len_tmp],"vma alloc fail size：");

    for (; i <= vma_alloc_order; i++) {
        len_tmp += len;
        len = sprintf(&page[len_tmp],"%lu\t", vma_alloc_err[i]);
    }
    len_tmp += len;
    len = sprintf(&page[len_tmp],"\n");
...
struct file_operations vma_alloc_err_proc_fops = {
	.read = vma_alloc_err_proc_read,
};

/******  End  ******/

/************** kswapd-cpuloading:START  ****************/
static ssize_t kswapd_loading_read(struct file *filp, char __user *buff, size_t count, loff_t *off)
{
        char page[256] = {0};
        int len = 0;

        len = sprintf(&page[len],"critical: %d\nheavy:%d\n",
            kc_account.critical_count, kc_account.heavy_count);
...
static const struct file_operations kswapd_loading_fops = {
       .read = kswapd_loading_read,
};
/************** kswapd-cpuloading:END  ****************/
...
/******  Stat  ******/
	...
	pentry = proc_create("scm_call", S_IRUGO, oppo_healthinfo, &proc_scmcall_fops);
	if(!pentry) {
		ohm_err("create scm_call proc failed.\n");
    }
	...
	pentry = proc_create("vma_alloc_err_account", S_IRWXUGO, oppo_healthinfo, &vma_alloc_err_proc_fops);
    if(!pentry) {
		ohm_err("create init_vma_alloc_accounts proc failed.\n");
		goto ERROR_INIT_VERSION;
	}
    
    pentry = proc_create("kswapd_loading", S_IRWXUGO, oppo_healthinfo, &kswapd_loading_fops);
	if(!pentry) {
		ohm_err("create  proc kswapd_loading failed.\n");
		goto ERROR_INIT_VERSION;
	}
